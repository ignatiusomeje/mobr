...
const UploadDetailsPop = () => {
  ...
  const [success, setSuccess] = useState<{visible:boolean, isDraft:boolean}>({
    visible:false,
    isDraft:false
  })
  return (
    <Dialog
      visible={visible}
      // visible
      className={`rounded-[20px] max-h-lvh h-full mb-10 mt-10 max-w-[1100px] w-full`}
      modal
      onHide={() => {
        if (!visible) return;
        setVisible(false);
      ...
      content={({ hide }) => (
        ...
        >
          ...
          <div
            className={`flex gap-[32px] flex-grow flex-col flex-1 overflow-y-scroll noScroll`}
          >
            <VehicleInformation />
            <VehicleFeatures />
            <Button
              className={`text-[#8D1510] font-square font-[400] text-[14px] leading-[22px] tracking-[0.25px]`}
            >
              DELETE CAR
            </Button>
            <div className={`flex items-center gap-[8px] .p-[24px]`}>
              <Button
                ...
                onClick={()=> setSuccess(prev => ({...prev, visible:true, isDraft: true}))}
              >
              ...
              <Button
                ...
                onClick={()=> setSuccess(prev => ({...prev, visible:true, isDraft: false}))}
              >
